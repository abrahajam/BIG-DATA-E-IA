...
class CityMultiAgentEnv(gym.Env):
    ...
    def _update_traffic_lights(self, force=False):
        ...
        if force or (self.traffic_timer % self.TRAFFIC_FREQ == 0):
            for r, c in self.traffic_spots:
                ...
                occupied = any(np.array_equal(
                    a['pos'], [r, c]) for a in self.agents)
                if occupied:
                    continue
                ...
                if rng < 0.4:
                    ...
                    self.current_map[r, c] = gen.ROAD_SLOW  # Amarillo
                else:
                    self.current_map[r, c] = gen.ROAD_JAM  # Rojo
        ...
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= nr < self.rows and 0 <= nc < self.cols:
                ...
                if val == gen.BUILDING:
                    ...
                    sensors.append(3)  # Bloqueado
                elif val == gen.ROAD_SLOW:
                    sensors.append(2)  # Lento
                else:
                    sensors.append(1)  # Libre
            else:
                sensors.append(0)
    ...
    def _generate_urgent_order(self):
        ...
        if random.random() < self.URGENT_PROB:
            if self.static_targets:
                ...
                self.tasks.append(
                    {'pos': target, 'urgente': True, 'asignado': False})
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            if (nr < 0 or nr >= self.rows or nc < 0 or nc >= self.cols or
                ...
                cost = -1
                if cell_type == gen.ROAD_SLOW:
                    cost = -2
                elif cell_type == gen.ROAD_JAM:
                    cost = -5
            ...
            # Objetivos
            dist = abs(agent['pos'][0]-agent['target'][0]) + \
                abs(agent['pos'][1]-agent['target'][1])

            # GPS Shaping (Crucial para mapa 30x30)
            if dist == 0:
                rewards[i] += 10  # Llegó al sub-objetivo
            else:
                rewards[i] -= 0.1  # Pequeño coste por tiempo
    ...
    def render(self):
        ...
        cmap = mcolors.ListedColormap(
            ['black', 'white', 'lightgreen', 'gray', 'gold', 'red', 'green'])
        ...
        for task in self.tasks:
            ...
            self.ax.plot(task['pos'][1], task['pos'][0], marker=marker,
                         color=color, markersize=10, alpha=alpha)
        ...
        for agent in self.agents:
            ...
            self.ax.plot(agent['pos'][1], agent['pos'][0], 's', color=c,
                         fillstyle=fill, markersize=8, markeredgewidth=2)

        p = len([t for t in self.tasks if not t['asignado']])
        self.ax.set_title(
            f"Mapa 30x30 | Pendientes: {p} | Agentes: {self.n_agentes}")
        plt.pause(0.01)
